...
本次工具调用只处理当前座位这一次主行动及其内部奖励选择，行动链到该主行动完成为止。后续自己的主行动依赖对手行动、补牌或新骰面时，保留资源目标和条件分支即可，不需要在当前请求中排定整条后续行动序列。Check 的并列路线必须是本次主行动的不同备选，不能把计划中的第二、第三次主行动当成并列路线。已比较的两条路线若没有新增能区分它们的事实或理由，就沿用已有取舍；出现实质新信息再调整。"""
